...
class cbzgpjts extends React.Component{
    state = {
        value: 0,
        open: false,
        //cbzgpjts: false,
    ...
    componentDidMount() {
        this.get_cbzgpjts_year();
        this.get_cbzgpjts_gk();
        this.update_cbzgpjts(this.state.year);
        this.update_cbzgpjts_jxsp(this.state.gk_index);

        //this.setState({cbzgpjts : true});
    }
    ...
    get_cbzgpjts_gk = () => {
        userService.get_cbzgpjts_gk()
            .then(
                cbzgpjts_gk => {
                    this.gks = cbzgpjts_gk.map((item, index) => { return item.gk; });
    ...
    update_cbzgpjts_jxsp = (gk_index) => {
        userService.get_cbzgpjts_jxsp(this.gks[gk_index])
            .then(
                cbzgpjts_jxsp => {
                    console.log(cbzgpjts_jxsp);
                    let yearArray = cbzgpjts_jxsp.map((item,index) => { return  item.year + '年'; });
                    let minArray = cbzgpjts_jxsp.map((item,index) => { return item.min; });
                    let avgArray = cbzgpjts_jxsp.map((item,index) => { return item.avg; });
                    let maxArray = cbzgpjts_jxsp.map((item,index) => { return item.max; });
                    let this_gk = cbzgpjts_jxsp.map((item,index) => { return item.jqpjqdts; });
                    var option = {
                        graphic: {
                            type: 'group',
                            rotation: Math.PI / 4,
    ...
    get_cbzgpjts_year = () => {
        userService.get_cbzgpjts_year()
            .then(
                cbzgpjts_year => { 
                    this.years = cbzgpjts_year.map((item,index) => { return  item.year; });
    ...
    update_cbzgpjts = (year) => {
        userService.get_cbzgpjts(year)
            .then(
                jqpjqdts => { 
                    let gkArray = jqpjqdts.map((item,index) => { return  item.gk; });
                    let cbzgpjtsArray = jqpjqdts.map((item,index) => { return item.jqpjqdts; });
                    var option = {
                        graphic: {
                            type: 'group',
                            rotation: Math.PI / 4,
...
export default withStyles(dashboardStyle)(cbzgpjts);
