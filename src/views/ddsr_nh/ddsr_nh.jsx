...
class ddsr_nh extends React.Component{
    state = {
        value: 0,
        open: false,
        //ddsr: false,
    ...
    update_ddsr_jxsp = (gk_index) => {
        userService.get_ddsr_nh_jxsp(this.gks[gk_index])
            .then(
                ddsr_jxsp => {
                    ...
                    let this_gk = ddsr_jxsp.map((item,index) => { return item.metric; });
                    var option = {
                        graphic: {
                            type: 'group',
                            rotation: Math.PI / 4,
    ...
    update_ddsr = (year) => {
        userService.get_ddsr_nh(year)
            .then(
                ddsr => { 
                    let gkArray = ddsr.map((item,index) => { return  item.gk; });
                    let ddsrArray = ddsr.map((item,index) => { return item.gkzjz; });
                    var option = {
                        graphic: {
                            type: 'group',
                            rotation: Math.PI / 4,
...
export default withStyles(dashboardStyle)(ddsr_nh);
