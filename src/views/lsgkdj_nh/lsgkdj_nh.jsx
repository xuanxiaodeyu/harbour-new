...
class lsgkdj_nh extends React.Component{
    state = {
        value: 0,
        open: false,
        //lsgkdj: false,
    ...
    componentWillMount() {
        this.get_lsgkdj_year();
        this.get_lsgkdj_gk();
        this.update_lsgkdj(this.state.year);
        this.update_lsgkdj_jxsp(this.state.gk_index);
        //this.setState({lsgkdj : true});
    }
    ...
    get_lsgkdj_gk = () => {
        userService.get_lsgkdj_nh_gk()
            .then(
                lsgkdj_gk => {
                    this.gks = lsgkdj_gk.map((item, index) => { return item.gk; });
    ...
    update_lsgkdj_jxsp = (gk_index) => {
        userService.get_lsgkdj_nh_jxsp(this.gks[gk_index])
            .then(
                lsgkdj_jxsp => {
                    let yearArray = lsgkdj_jxsp.map((item,index) => { return  item.year + '年'; });
                    let minArray = lsgkdj_jxsp.map((item,index) => { return item.min; });
                    let avgArray = lsgkdj_jxsp.map((item,index) => { return item.avg; });
                    let maxArray = lsgkdj_jxsp.map((item,index) => { return item.max; });
                    let this_gk = lsgkdj_jxsp.map((item,index) => { return item.lsgkdj; });
                    var option = {
                        graphic: {
                            type: 'group',
                            rotation: Math.PI / 4,
    ...
    get_lsgkdj_year = () => {
        userService.get_lsgkdj_nh_year()
            .then(
                lsgkdj_year => { 
                    this.years = lsgkdj_year.map((item,index) => { return  item.year; });
    ...
    update_lsgkdj = (year) => {
        userService.get_lsgkdj_nh(year)
            .then(
                lsgkdj => { 
                    let gkArray = lsgkdj.map((item,index) => { return  item.gk; });
                    let lsgkdjArray = lsgkdj.map((item,index) => { return item.lsgkdj; });
                    var option = {
                        graphic: {
                            type: 'group',
                            rotation: Math.PI / 4,
...
export default withStyles(dashboardStyle)(lsgkdj_nh);
